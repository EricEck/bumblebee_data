<div>
    ...
    <!--  Form Title -->
        <div class="px-4 sm:px-0">
            <h3 class="text-lg italic font-medium leading-6 text-black ">{{ $create_new ? 'New Component Location' : 'Edit Component Location' }}</h3>
        </div>
        <!-- Form Container -->
        <div class="overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <form >
                ...
                <div class="sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:border-b sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-0.5 sm:mt-0 sm:col-span-4">
                        <input type="text"
                               ...
                               wire:model.lazy="componentLocation.name"
                               wire:change="changed"
                               placeholder="name of location (hint: make it easy to remember)"
                               {{ $allow_edit ?? '' ?  '' : 'disabled'}}
                ...
                <div class="sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:border-b sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-0.5 sm:mt-0 sm:col-span-4">
                        <textarea
                            wire:model.lazy="componentLocation.description"
                            wire:change="changed"
                            placeholder="description of the location (hint: finding it, critical features, etc.)"
                            {{ $allow_edit ?? '' ?  '' : 'disabled'}}
                            class="{{ $allow_edit ?  'bg-white' : 'bg-indigo-50'   }} mt-1 px-3 py-3 text-black block w-full py-2 px-3 text-sm font-medium leading-5 text-gray-700 rounded-lg border border-gray-200 sm:mt-px sm:pt-2 shadow-sm border-gray-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5 ">
                        </textarea>
                    </div>
