<x-forms.form>
    ...
    <x-slot name="form_title">
        {{ $create_new ? 'New Body of Water Data' : 'Edit Body of Water Data' }}
    </x-slot>
            ...
            <div class="sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                ...
                <div class="mt-0.5 sm:mt-0 sm:col-span-4">
                    <select wire:model.lazy="bodyOfWater.pool_owner_id"
                        ...
                        <option disabled selected value="0">-- Name of the Primary Owner (required)</option>

                        @foreach($primaryOwnersList as $primaryOwner)
                            <option value="{{$primaryOwner->user->id}}">{{$primaryOwner->user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
